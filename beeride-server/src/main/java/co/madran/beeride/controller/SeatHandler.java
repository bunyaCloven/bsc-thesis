...
public class SeatHandler {
  ...
  public String signToCarpool(@RequestParam Long id, @RequestParam String username) {
    ...
    if (seatRepository.findByUserAndCarpool(user, carpool) != null) {
      ...
      response.addProperty("message", "You have already signed to this carpool.");
  ...
  public String getCarpools(@RequestParam String username, @RequestParam Integer page,
      @RequestParam Integer limit) {
    JsonObject response = new JsonObject();
    User user = userRepository.findByUsername(username);
    response.add("data",
        gson.toJsonTree(seatRepository.findCarpools(user, new PageRequest(page - 1, limit))));
    response.addProperty("success", true);
    return response.toString();
  }
  ...
  public String getSeatCarpool(@PathVariable Long id, @RequestParam String username) {
  ...
  public String removeFromCarpool(@RequestParam String username, @RequestParam Long id) {
    JsonObject response = new JsonObject();
    User user = userRepository.findByUsername(username);
    Carpool carpool = carpoolRepository.findOne(id);
    carpool.setCurrentPassengers(carpool.getCurrentPassengers() - 1);
